...
import { getUserAuthByEmail } from "@/lib/repositories/users";
 ...
 * leave the email field blank at /login. This is what lets the person who
 * deploys this app get in and approve the very first real account without
 * a chicken-and-egg problem, and stays as an emergency door if the users
 * table is ever unreachable. APP_ACCESS_PASSWORD now grants the "admin"
 * role (user management included), not just "editor".
 */
export const authOptions: AuthOptions = {
  providers: [
    CredentialsProvider({
      name: "Password",
      credentials: {
        email: { label: "Email (leave blank for the shared access password)", type: "text" },
